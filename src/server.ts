...
import * as sccBrokerClient from 'scc-broker-client';

const ENVIRONMENT = process.env.ENV || 'dev';
const SOCKETCLUSTER_PORT = parseInt(process.env.SOCKETCLUSTER_PORT!) || 8000;
...
const SCC_STATE_SERVER_PORT = parseInt(process.env.SCC_STATE_SERVER_PORT!) || undefined;
const SCC_MAPPING_ENGINE = process.env.SCC_MAPPING_ENGINE || undefined;
const SCC_CLIENT_POOL_SIZE = parseInt(process.env.SCC_CLIENT_POOL_SIZE!) || undefined;
...
//TODO: Remove this : serve static files, the static files muts be served from the Cloud CDN, so that they can be scaled differently as compared to this
expressApp.use(serveStatic(path.resolve(__dirname, APP_ENV==="PROD"?'../public1':'../public')));
...
if (SCC_STATE_SERVER_HOST) {
  ...
  let sccClient = sccBrokerClient.attach(agServer.brokerEngine, {
    ...
    mappingEngine: SCC_MAPPING_ENGINE as any,
